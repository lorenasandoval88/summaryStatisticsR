...
MAKE.MISS.STAT <-function(box_id){
  ...
  cols <- c("ethnicityClass_European" = 0,"ethnicityClass_Hispanic" = 0,
            ...
            "ethnicityClass_South-East Asian"=0,"ethnicityClass_Other"=0, "status_control"=0)
  results<-add_column(results, !!!cols[setdiff(names(cols), names(results))])
...
box_id1= "769464052920"
box_id2= "769693462935"
...
old_df_summ_stat = box_read(647352716008) # old summstat file, DO NOT CHANGE
...
#df8_summ_stat = MAKE.SUMMSTAT(box_id8)

df_final_summ_stat = rbind(old_df_summ_stat, df1_summ_stat, df2_summ_stat)# old_df_summ_stat,df2_summ_stat,df3_summ_stat) # COMBINE DFs IF MORE THAN ONE or adding to old summary statistics file
...
#df8_miss_stat = MAKE.MISS.STAT(box_id8)

df_final_miss_stat = rbind(old_df_miss_stat, df1_miss_stat, df2_miss_stat)# old_df_miss_stat,df2_miss_stat,df3_miss_stat) 
